...
new_vim_color = []
usage = "Adds gui compatibility to cterm-only themes and the other way\n\
Usage:\n\
\tvim_cterm2gui2cterm.py --to (cterm|gvim) colorscheme.vim" 
...
if(args.to == "gvim" or args.to =="cterm"):
    if(args.to == "gvim"):
        for line in args.f:
            _tmp = line.split()
            _tmp_line = []
            for _t in _tmp:
                if 'ctermfg=' in _t:
                    html_color = _t.split('ctermfg=')[1]
                    try:
                        if(html_color and html_color.isdigit()):
                            c = xtermMap[html_color]
                            _tmp_line.append("guifg="+c)
                    except IndexError:
                        continue
                if 'ctermbg=' in _t:
                    html_color = _t.split('ctermbg=')[1]
                    try:
                        if(html_color and html_color.isdigit()):
                            c = xtermMap[html_color]
                            _tmp_line.append("guibg="+c)
                    except IndexError:
                        continue
                if 'cterm=' in _t:
                    _term = _t.split('cterm=')[1]
                    _tmp_line.append("gui=%s" % _term)
                _tmp_line.append(_t)
